...
exports.checkTopupOrderStatus = checkTopupOrderStatus;
exports.fetchProviderProfile = fetchProviderProfile;
...
// ─────────────────────────────────────────────────────────────────────────────
function resolveLiveProductCode(gameSlug, packageName, amount) {
    ...
    if (slug.includes('free-fire') || slug.includes('freefire')) {
        ...
        if (name.includes('lvup') || name.includes('level up')) {
            if (name.includes('l6') || name.includes('level 6'))
                return 'FREEFIRE_SGMY_Level_Up_Package___Level_6';
            if (name.includes('l10') || name.includes('level 10'))
                return 'FREEFIRE_SGMY_Level_Up_Package___Level_10';
            if (name.includes('l15') || name.includes('level 15'))
                return 'FREEFIRE_SGMY_Level_Up_Package___Level_15';
            if (name.includes('l20') || name.includes('level 20'))
                return 'FREEFIRE_SGMY_Level_Up_Package___Level_20';
            if (name.includes('l25') || name.includes('level 25'))
                return 'FREEFIRE_SGMY_Level_Up_Package___Level_25';
            if (name.includes('l30') || name.includes('level 30'))
                return 'FREEFIRE_SGMY_Level_Up_Package___Level_30';
            return 'FREEFIRE_SGMY_Level_Up_Package___Level_6';
        }
        // Diamonds count
        if (slug.includes('global')) {
            if (amt <= 150)
                return 'FREEFIRE_GLOBAL_110';
            if (amt <= 400)
                return 'FREEFIRE_GLOBAL_341';
            if (amt <= 700)
                return 'FREEFIRE_GLOBAL_572';
            if (amt <= 1500)
                return 'FREEFIRE_GLOBAL_1166';
            if (amt <= 3000)
                return 'FREEFIRE_GLOBAL_2398';
    ...
    if (slug.includes('mobile-legend') || slug.includes('mlbb') || slug.includes('moonton')) {
        if (name.includes('twilight'))
            return 'MLBB_Twilight';
        if (name.includes('w.elite') || (name.includes('weekly') && name.includes('elite')))
            return 'MLBB_Weekly_Elite_Pack';
        if (name.includes('m.epic') || (name.includes('monthly') && name.includes('epic')))
            return 'MLBB_Monthly_Epic';
        if (name.includes('w.pass') || name.includes('weekly')) {
            if (name.includes('2x') || name.includes('2×'))
                return 'MLBB_Weekly_X2';
            if (name.includes('3x') || name.includes('3×'))
                return 'MLBB_Weekly_X3';
            if (name.includes('4x') || name.includes('4×'))
                return 'MLBB_Weekly_X4';
            if (name.includes('5x') || name.includes('5×'))
                return 'MLBB_Weekly_X5';
            if (name.includes('10x') || name.includes('10×'))
                return 'MLBB_Weekly_X10';
            return 'MLBB_Weekly';
        }
        if (name.includes('monthly'))
            return 'MLBB_Monthly_Elite_Pack';
    ...
    if (slug.includes('pubg')) {
        if (name.includes('elite') && name.includes('plus'))
            return 'PUBGM_Elite_Pass_Plus_LV1_100';
        if (name.includes('elite'))
            return 'PUBGM_Elite_Pass_LV1_100';
        if (name.includes('prime') && name.includes('plus'))
            return 'PUBGM_Prime_Plus_1_Month';
        if (name.includes('prime'))
            return 'PUBGM_Prime_1_Month';
        if (amt <= 80)
            return 'PUBGM_60';
        if (amt <= 150)
            return 'PUBGM_60';
    ...
    if (slug.includes('honor-of-kings') || slug.includes('hok')) {
        if (name.includes('weekly') && name.includes('plus'))
            return 'HOK_Weekly_Card_Plus';
        if (name.includes('weekly'))
            return 'HOK_Weekly_Card';
        if (amt <= 30)
            return 'HOK_16';
        if (amt <= 100)
            return 'HOK_80';
    ...
    if (slug.includes('farlight')) {
        if (amt <= 8)
            return 'FARLIGHT84_5';
        if (amt <= 15)
            return 'FARLIGHT84_10';
        if (amt <= 25)
            return 'FARLIGHT84_20';
        if (amt <= 45)
            return 'FARLIGHT84_40';
        if (amt <= 55)
            return 'FARLIGHT84_50';
        if (amt <= 70)
            return 'FARLIGHT84_60';
        if (amt <= 90)
            return 'FARLIGHT84_80';
        if (amt <= 120)
            return 'FARLIGHT84_100';
        if (amt <= 180)
            return 'FARLIGHT84_165';
        if (amt <= 250)
            return 'FARLIGHT84_220';
        if (amt <= 400)
            return 'FARLIGHT84_330';
        if (amt <= 1000)
            return 'FARLIGHT84_880';
        if (amt <= 2500)
            return 'FARLIGHT84_2240';
...
async function deliverTopup(gameSlug, playerId, playerZoneId, packageName, price, orderTxnId, productCode, packageAmount) {
    ...
    console.warn(`[VNGZZ2GAME API] ⚠️ No product_code resolved for "${packageName}" (${gameSlug}). ` +
